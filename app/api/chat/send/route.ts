...
export async function POST(request: NextRequest) {
  try {
    ...
    const state = conversationManager.updateState(
      sessionId,
      userMessage,
      contextUpdate
    );
    ...
    const recent = state.conversationHistory
      .slice(-6)
    ...
    const contextualInstructions = `Stage: ${
      state.legalContext.currentStage
    }\nIncident: ${state.currentCase.incidentType}\nApplicableLaws: ${
      state.legalContext.applicableLaws.join("; ") || "N/A"
    }\nCaseSummary: ${summary}`;
    ...
    if (
      ...
      state.legalContext.applicableLaws.length > 0
    ) {
      references = {
        laws: cleanAndDedupe(state.legalContext.applicableLaws.slice(0, 5)),
    ...
    conversationManager.updateState(sessionId, assistantMessage as any, {
      newFacts: [],
      timelineUpdates: [],
      legalIssues: [],
      evidence: [],
    ...
    saveQaResult({
      ...
      legal: qa.legalRelevanceScore,
      context: qa.contextUsageScore,
      flags: qa.flags,
